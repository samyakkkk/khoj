...
class TextToEntries(ABC):
    ...
    def process_entity_batch(entities: List[Dict[str, str]]) -> Dict[str, List[str]]:
        ...
        persons = {entity["text"].lower() for entity in entities if entity["label"] == "PERSON"}
        teams = {entity["text"].lower() for entity in entities if entity["label"] == "ORG"}
        locations = {entity["text"].lower() for entity in entities if entity["label"] in ["GPE", "LOC", "FAC"]}
        # Add unclassified proper nouns to projects
        projects = {entity["text"].lower() for entity in entities if entity["label"] == "PROPN"}
        if state.verbose > 1:
            logger.debug(
                f"Extracted\nPersons: {persons}\nTeams: {teams}\nLocations: {locations}\nProjects: {projects}\n\n"
            )
        return {"people": list(persons), "teams": list(teams), "locations": list(locations), "projects": list(projects)}
    ...
    def extract_ner(texts: List[str], batch_size: int = 32) -> List[Dict[str, List[str]]]:
        ...
        with timer("Infer named entities in entry", logger, log_level=logging.DEBUG):
            for doc in tqdm(
                ...
                # Get nouns that aren't already entities
                entity_spans = set(doc.ents)
                other_entities = [
                    {"text": token.text, "label": "PROPN"}
                    for token in doc
                    if (
                        token.pos_ in ["PROPN"]  # Proper nouns and nouns
                        and not any(token in span for span in entity_spans)  # Not already an entity
                        and not token.is_stop  # Not a stopword
                    )
                ]
                predictions = standard_entities + other_entities
